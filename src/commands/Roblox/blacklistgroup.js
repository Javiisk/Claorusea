...
export default {
  ...
  async execute(interaction) {
    ...
    if (!deferSuccess) {
      logger.warn('BlacklistGroup interaction defer failed', {
        userId: interaction.user.id,
        guildId: interaction.guildId,
        commandName: 'blacklistgroup',
      });
      return;
    }
    ...
    try {
      ...
      if (!groupInfo) {
        return await InteractionHelper.safeEditReply(interaction, {
          content: '❌ Group not found on Roblox.',
        });
      }
      ...
      if (action === 'add') {
        if (exists) {
          return await InteractionHelper.safeEditReply(interaction, {
            content: `⚠️ **${groupInfo.name}** is already blacklisted.`,
          });
        }
      ...
      if (action === 'remove') {
        if (!exists) {
          return await InteractionHelper.safeEditReply(interaction, {
            content: `⚠️ **${groupInfo.name}** is not blacklisted.`,
          });
        }
